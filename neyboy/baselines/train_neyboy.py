...
from cmd_util import neyboy_arg_parser, make_neyboy_environment
...
def train(env_id, learning_rate, num_epoch, buffer_size, batch_size, num_timesteps, num_workers, seed, policy, load_path):
    ...
    def make_env(rank):
        def _thunk():
            env = make_neyboy_environment(env_id, seed, rank, allow_early_resets=True)
            return env
        return _thunk

    env = DummyVecEnv([make_env(i) for i in range(num_workers)])
